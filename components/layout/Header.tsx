  ...
  useEffect(() => {
    ...
    if (menuOpen) {
      ...
      tl.set(menu, { display: "flex" })
        .fromTo(menu, { clipPath: "circle(0% at calc(100% - 2.5rem) 2.5rem)" }, { clipPath: "circle(150% at calc(100% - 2.5rem) 2.5rem)", duration: 0.8 })
        .fromTo(links.children, { opacity: 0, x: 40 }, { opacity: 1, x: 0, stagger: 0.07, duration: 0.5 }, "-=0.3");
    } else {
      document.body.style.overflow = "";
      tl.to(links.children, { opacity: 0, x: -20, stagger: 0.04, duration: 0.25 })
        .to(menu, { clipPath: "circle(0% at calc(100% - 2.5rem) 2.5rem)", duration: 0.6, onComplete: () => { menu.style.display = "none"; } }, "-=0.1");
    }
